...
const TelephonyPlugin = GObject.registerClass({
    ...
    _init(device) {
        ...
        this._window = null;
        this._activeBluetoothCall = false;
        this._callWatchId = 0;
    ...
    _notifyEvent(packet) {
        ...
        if (packet.body.event === 'ringing') {
            ...
            if (this._bluetoothTelephony?.canControl(this.device)) {
                buttons.unshift({
                    action: 'answerCall',
                    // TRANSLATORS: Answer the actively ringing call
                    label: _('Answer'),
            ...
            if (this._window !== null) {
                this._window.showCall(packet.body.phoneNumber ?? sender, 'incoming',
                    null, 'close');
            }
        ...
        if (packet.body.event === 'talking') {
            ...
            if (this._window !== null) {
                this._window.showCall(packet.body.phoneNumber ?? sender, 'talking',
                    null);
            }
        }
    ...
    async _dial(number) {
        ...
        if (this._bluetoothTelephony !== undefined) {
            ...
            try {
                ...
                if (callPath) {
                    this._activeBluetoothCall = true;
